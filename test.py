...
if API_KEY_1:
    try:
        client = genai.Client(api_key=API_KEY_1, http_options={'api_version': 'v1alpha'})
        print("✅ Using API_KEY_1")
    except Exception:
        if API_KEY_2:
            try:
                client = genai.Client(api_key=API_KEY_2, http_options={'api_version': 'v1alpha'})
                print("✅ Using API_KEY_2")
            except Exception:
                if API_KEY_3:
                    try:
                        client = genai.Client(api_key=API_KEY_3, http_options={'api_version': 'v1alpha'})
                        print("✅ Using API_KEY_3")
                    except Exception:
                                pass
else:
...
if client is None:
    raise RuntimeError("No valid API key worked. Please check your .env file.")
